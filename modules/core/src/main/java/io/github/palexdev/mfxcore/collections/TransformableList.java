...
/// A `TransformableList` is a particular type of List which wraps another
...
/// <pre>
///
/// `// Let's say I have this ObservableListObservableList<String> source = FXCollections.observableArrayList("A", "B", "C"):// Now let's say I want to sort this list in reverse order (CBA) and that// for some reason I still want A to be the element at index 0, B-1 and C-2// This is exactly the purpose of the TransformableList...TransformableList<String> transformed = new TransformableList<>(source);transformed.setSorter(Comparator.reverseOrder());// Now that the order is (CBA) let's see how the list behaves:transformed.get(0); // Returns Ctransformed.indexOf("C"); // Returns 2, the index is retrieved in the source listtransformed.viewToSource(0); // Returns 2, it maps an index of the transformed list to the index of the source list, at 0 we have C which is at index 2 in the source listtransformed.sourceToView(0); // Also returns 2, it maps an index of the source list to the index of the transformed list, at 0 we have C which is at index 2 in the transformed list// To better see its behavior try to sort and filter the list at the same time.// You'll notice that sometimes sourceToView will return a negative index because the item is not in the transformed list (after a filter operation)`
/// </pre>
///
/// Check [#computeIndexes()] documentation to see how indexes are calculated.
///
///
/// **IMPORTANT:** If using a reversed comparator please use [#setComparator(Comparator,boolean)] with 'true' as argument,
...
// FIXME remove this and backport RefineList
public class TransformableList<T> extends TransformationList<T, T> {
    ...
    /// Responsible for updating the transformed indexes when the predicate or the comparator change.
    private void update() {
        indexes.clear();
    ...
    /// the transformed indexes by creating a [SortedMap] and mapping every index from 0 to source size
    /// to its item. Before mapping, items are filtered with the given predicate, [#predicateProperty()].
    /// Before returning, the map's entry set is sorted by its values with the given comparator, [#comparatorProperty()].
    ...
    /// a special flag is set to true and [#sourceToView(int)] behaves accordingly.
    ...
    /// Communicates to the transformed list, specifically to [#getViewIndex(int)],
    ...
    /// This means transformations due to [#predicateProperty()] or [#comparatorProperty()]
