...
public interface IBinding<T> {
    ...
    ///
    /// Unidirectional bindings will throw an [UnsupportedOperationException].
    ...
    /// This method is responsible for "deactivating" this binding, but unlike [#unbind()]
    /// that still lets you re-set/re-use the binding, this will stop you from re-activating it.
    void dispose();

    /// @return the [Target] object responsible for keeping the binding's target
    /// [ObservableValue] and other useful info.
    Target<T> getTarget();

    /// @return the state of the binding, see [BindingState]
    BindingState state();

    /// Checks if the state of the binding is [BindingState#DISPOSED].
    default boolean isDisposed() {
        return state() == BindingState.DISPOSED;
    }

    /// Checks if the binding's state is [BindingState#BOUND].
    ///
    /// **Note** that this will only check the binding's state and not the target.
    /// A full check should also consider calling [MFXBindings#isBound(Target)].
