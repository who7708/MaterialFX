...
/// A more flexible alternative to [FilteredList] and [SortedList]. This combines both into a single class while still
/// allowing modifications to the source list.
///
/// All operations except for observability are performed on the source list. Listeners are added on the `view` list.
///
/// #### View List
///
/// Combining the capabilities of [FilteredList] and [SortedList] is as simple as making the first operate on the source
/// and the latter on the filtered one. So, the `view` is the [SortedList] because it is also filtered.
public class RefineList<T> implements ObservableList<T> {
    ...
    /// Delegates to [FilteredList#getViewIndex(int)] if the predicate is not `null` otherwise to [SortedList#getViewIndex(int)].
